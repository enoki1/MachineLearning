...
for i in range(sweller_n):
    if swx[i] == math.floor(swx[i]) and swy[i] == math.floor(swy[i]):
        print("minimal distance for dweller number" + " " + str(i + 1) + ":" + str(m_distance1(swx[i], swy[i])))
        print("the school is:")
    ...
    if swx[i] == math.floor(swx[i]) and swy[i] != math.floor(swy[i]):
        ...
        if min1 + abs(swy[i] - math.floor(swy[i])) < min2 + abs(math.floor(swy[i] + 1) - swy[i]):
            print("minimal distance for dweller number" + " " + str(i + 1) +  ": " + str(min1 + swy[i] - math.floor(swy[i])))
            print("the school is:")
            m_distance2(swx[i], math.floor(swy[i]))
        else:
            print("minimal distance for dweller number" + " " + str(i + 1) + ": " + str(min2 - swy[i] + math.floor(swy[i] + 1)))
            print("the school is:")
    ...
    if swx[i] != math.floor(swx[i]) and swy[i] == math.floor(swy[i]):
        ...
        if min1 + abs(swx[i] - math.floor(swx[i])) < min2 + abs(math.floor(swx[i] + 1) - swx[i]):
            print("minimal distance for dweller number" + " " + str(i + 1) + ": " + str(min1 + abs(swx[i] - math.floor(swx[i]))))
            print("the school is:")
            m_distance2(math.floor(swx[i]),swy[i])
        else:
            print("minimal distance for dweller number" + " " + str(i + 1) +  ": " + str(min2 - swx[i] + math.floor(swx[i] + 1)))
