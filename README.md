...
Вот тест на котором быстро можно проверить ответ(генерировал одинаковые последовательности длиной 1000, у одной в конце удалил один символ)
![](images/нидлман.png)
 ...
 заданы прямые k1 и k2 штук(заданы коодринаты школ) далее набор координат жителей. найти ближайшую школу для человека(ходим только по сетке, когда мы в одной клетке то можем шагнуть перпендикулярно)
  ...
  # Начальные условия:
   ...
   2) Сетка равномерная
   3) Метрика Манхеттен
   4) людей выбираю внутри клеток(а может и в самом узле)
   
 # Описание метода
   
  Научиться быстро решать эту задачу у меня не получилось, в конечном итоге все свелось к нехитрому перебору и геометрии на плоскости. Пытался реализовать алгоритм Форчуна, но сделать это без каких-либо доп.библиотек не получилось. Реализацию с бинпоиском я не придумал, я могу запустить для каждого человека 4 бинпоиска, чтобы найти все ближайшие слева и справа школы, поочередно находя ближайшие школы по каждой координате.
  Но минимум из этих четырех вариантов не гарантирует минимальной расстояния от школы до человека. 
  Весь код: https://github.com/enoki1/MachineLearning/blob/main/Schools.py
 # Тесты
 ![](images/тестышколы1.png)
 
 очевидно данный тест верно определил школы
 
 В этом тесте я просто поставил всех людей в линию, в самих узлах, а школы на параллельной к людям линии и тоже подряд, ответ очевиден:
 
 
 ![](images/тестышколы2.png)
...
#  3)Кластеризация на плоскости.
  ...
  ![](images/clast1.png)
  ![](images/clast2.png)
# 4) Регрессия (-----) PS: смотреть в командных задачах!
...
![](images/kNN2.png)
 ...
 # Результаты
  ...
  # 2.1) Еще раз кластерный анализ.
      На этот раз я буду рассматривать кластерный анализ на данных с первой командной задачи, про ту же игру PUBG.
      весь код: https://github.com/enoki1/MachineLearning/blob/main/Claster2.py
   # Описание
      Попробую кластеризировать координаты умерших игроков.
   # Условия
      рассматривал лишь карту ERANGEL.
   # Результаты  
   Вот результат кластеризации
    ![](images/класт1.png) 
   Попробуем нанести данные точки на карту игры:
   ![](images/pubg1.png)
  ...
  # Вывод
      Наше разбиение на классы вполне адекватно показало, где чаще всего умирают игроки. Очевидно, около больших скоплений домов
  # 4) Регрессия
        Скачал с Kaggle базу данных по COVID-19, попробовал построить регрессионные модели для данного датафрейма.
        весь код: https://github.com/enoki1/MachineLearning/blob/main/regress2.py
   # Описание
   Датафрейм выглядит вот так:
   ![](images/ковид-1.png)
   
   Первое что пришло в голову - попробовать попредугадывать число зараженных людей взависимости от даты.
   Поэтому пользовался лишь первыми двумя столбцами датафрейма, другие пока даже не смотрел.
  # Условие эксперимента
   Начал высчитывать данные с первого дня(какого-то условного первого дня), так легче работать с массивами и прочим.
   Рассматривал 1000 дней из датафрейма.
   В задаче мне самому требовалось взглянув на данные попробовать предугадать регрессионную модель, а затем проверить ее.
   Посмотрев на данные я понял что скорее всего регрессионная модель будет выглядеть экспоненциально. Тогда для меня это было практически очевидно,
   потому что ЧУМА, например, заражала экспоненциально, да и большинство вирусов растут по экспоненте. Мое предположение - экспонента.
  
 # Результаты
  Я реализовал 5 типов регрессий: Линейную, Полином 2й степени, Полином 3й степени, Показательную(a0 * a1^x),Экспоненциальную(a0 * e^(a1x)
  К слову, показательная и экспоненциальная регрессии получились практически одинаковыми, вот сами результаты:
  
  Внизу я вывел коэффициенты соответствующих функций. Это тестовый вариант регрессии, взял поменьше дней(легче разглядеть графики).

  ![](images/ковид0.png)
  
  Здесь уже данных больше:
  
  ![](images/ковид1.png)
  
  Совсем отдельно я вывел экспоненциальную модель, как свое предположение.
  На этом же скрине я посчитал отклонения всех моделей от настоящих данных и понял,
  что лучшая модель - кубический полином. Это весьма странно, даже на графике видно, что дальше вирус начинает расти линейно,
  не очень верится в это...(я ожидал увидеть экспоненту). А получилось, что к концу экспонента растет намного быстрее, чем наши данные.
  ![](images/ковид2.png)
